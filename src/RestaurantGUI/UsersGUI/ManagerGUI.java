...
public final class ManagerGUI {
    ...
    public void managerDashBoard(Manager manager, UsersVector usersVector,
        ...
        searchUserB = setUpButton("Search User");
        setUpSearchUserBAction(usersVector);
        ...
        buttonsPane.add(searchUserB, 2, 2);
...
// In ManagerGUI.java
    ...
    private void setUpSearchUserBAction(UsersVector usersVector) {
        searchUserB.setOnAction(e -> {
            TextInputDialog dialog = new TextInputDialog();
            dialog.setTitle("Search User");
            dialog.setHeaderText("Enter username to search");
            dialog.setContentText("Username:");
            dialog.getDialogPane().setStyle(getDialogStyle());

            Optional<String> result = dialog.showAndWait();
            result.ifPresent(username -> {
                Users user = usersVector.searchByUsername(username);
                if (user != null) {
                    ...
                    alert.getDialogPane().setStyle(getAlertStyle());
                    alert.showAndWait();
                } else {
                    Alert alert = new Alert(Alert.AlertType.ERROR);
                    alert.setTitle("User Not Found");
                    alert.setHeaderText(null);
                    alert.setContentText("No user found with username: " + username);
                    alert.getDialogPane().setStyle(getAlertStyle());
                    alert.showAndWait();
                }
            });
